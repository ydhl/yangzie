...
class YDHL_HttpClient{
	...
	public function get($url, $params = array())
	{
		...
		log4web($url, "httpclient");
		return $this->http($url,'GET');
	}

	function post($url, $params, $multi = false) {
		$query = "";
		if( is_array($params)){
			if($multi)
	...
	function http($url, $method, $postfields = NULL, $multi = false) {
		...
		$this->url = $url;
		if(strtoupper($method) == "POST" && $url!="http://log4web.yidianhulian.com/log.json"){
			ob_start();
			var_dump($curl_info);
			var_dump($response);
			var_dump($postfields);
			
			log4web(ob_get_clean() , $url);
		}
		curl_close ($ci);
		return $response;
	}

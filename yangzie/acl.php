<?php
class ACL extends YangzieObject{
	private $aros_acos;
	private $acos;
	...
	public function check_byname($aroname, $aconame){
		...
		if(is_array($aroname)){//当前用户有多个角色
			foreach ($aroname as $value) {
				$check_rst = $this->_check_permission($value, $aconame);
				if($check_rst)return true;
			}
			return false;
		}else{
        	return $this->_check_permission($aroname, $aconame);
		}
	}
	
	private function _check_permission($aroname, $aconame)
	{
        if (!trim($aroname)) {
            return false;
        }
